...
2. `lid-macros` ships second and must reproduce the hand-expanded registrations
   ...
   equivalence. `lid` puts `extern crate self as lid;` in its root so the
   emitted `::lid::...` paths resolve inside `lid` itself.
3. Tracing then spreads through `lid`'s own code, the intent-graph checks land,
...
- The canary equivalence test (Goal 2) stays green across `lid-macros` changes.
