...
the hand-expanded form from `docs/intent/registry/lld.md` exactly, and they
...
`lid-macros` is a proc-macro crate: it executes at compile time on the host,
...
| Macro | Kind | On | Emits |
|---|---|---|---|
| `Spec` | derive | unit struct | `impl lid::Spec` with `NAME = concat!(module_path!(), "::", ident)`; a `SPECS` registration in a `const _` block |
...
2. **Swap** (implementation phase): the hand expansions in `lid` are replaced
