...
| Form of the earned rules | One section in `SKILL.md`, as a when → do-this table: the moment and the check, without the evidence | A companion `examples.md` in the skill directory read on demand; worked examples inline at each phase | A rule read at the moment it applies is the only kind that works, and the moment is a phase stop or a gate firing, both already in `SKILL.md`; a companion file is one more thing `sync` must carry and an agent may not open. Inline-per-phase spreads a dozen rules across the walk and doubles its length. Revisit if the section outgrows a screen. |
| Where the canonical skill lives | `lid-rs/skill/SKILL.md`, inside the crate that its toolchain-half describes; working copies are synced, never edited | `.claude/skills/lid-rs/SKILL.md` as canonical with a template copy in the tool (the 0.1 arrangement); a plugin | The skill's mechanics are the crate's mechanics; shipping it with the crate makes "update the dependency" the update. A copy in the tool binary describes whatever version the tool happens to be. The plugin remains the destination for the process half once the seam is found. |
