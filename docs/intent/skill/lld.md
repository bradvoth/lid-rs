...
6. **Working state** — a slice's branch and its phase-tagged commits are the
   ...
   instead of one squashed diff. The tag is also the gate's dispatch key:
   the `commit-msg` hook runs phase N's check on a `phase N:` commit
   (`cargo-lid-rs/docs/intent/phase/lld.md`), so the skill tells the agent
   to commit, never to decide whether a check runs.
7. **The other mode** — the same branch can be built unattended by the
   `lid-rs` workflow, with a clean agent in the reviewer's seat at each
   stop; the skill names it, states that the two modes' output is
   indistinguishable in git, and does not launch it.
...
| Who runs the phase checks | The `commit-msg` hook, keyed on the `phase N:` tag; the skill only says "commit" | The agent runs each phase's check itself and pastes the output (the 0.2.1 arrangement); a `PreToolUse` hook on `git commit` | The `dmdr` evidence is a list of checks the agent was told to run and did not: a red run skipped under a reused waiver, "commit anyway" offered at a failed gate. A check the agent runs is a check the agent can skip; the hook runs it whether or not the agent meant to. Details and alternatives in `cargo-lid-rs/docs/intent/phase/lld.md`. |
