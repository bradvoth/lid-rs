# Summary

[LID-rs](index.md)

# The Specification

- [The premise](spec/premise.md)
- [What this is for](spec/purpose.md)
- [Design constraints](spec/constraints.md)
- [Concept → mechanism](spec/mapping.md)
- [The twenty-six checks](spec/gates.md)
- [The registry](spec/registry.md)
- [Traced and untraced code](spec/traced.md)
- [Configuration](spec/configuration.md)
- [The flow](spec/flow.md)
- [Worked example: login](spec/example-login.md)
- [Worked example: settings](spec/example-settings.md)
- [Repo layout and adoption](spec/layout.md)
- [Honest limits](spec/limits.md)
- [Bootstrap checklist](spec/bootstrap.md)

# The Reference Implementation

- [Workspace HLD](impl/hld.md)
- [LLD: the registry](impl/lld-registry.md)
- [LLD: citation macros](impl/lld-macros.md)
- [LLD: intent-graph checks](impl/lld-intent-graph.md)
- [LLD: cargo-lid-rs](impl/lld-cargo-lid-rs.md)
- [LLD: init and new](impl/lld-init.md)
- [LLD: sync](impl/lld-sync.md)
- [LLD: the gate self-test](impl/lld-xtask.md)
- [LLD: the operating skill](impl/lld-skill.md)
- [LLD: the book](impl/lld-book.md)
- [LLD: publishability](impl/lld-publish.md)
- [LLD: the phase gate](impl/lld-phase.md)
- [LLD: the canopy agent](impl/lld-headless-canopy-agent.md)
- [LLD: reviewing an LLD](impl/lld-lld-review.md)
- [LLD: the LLD coach](impl/lld-coach.md)

# Operating It

- [The skill](skill.md)
- [The pipeline](pipeline.md)
