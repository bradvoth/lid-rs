...
//! because downstream crates link `lid-rs` compiled without `cfg(test)`, and the
...
mod tests {
    ...
    fn linked_registrations_are_enumerable() {
        ...
        assert!(lid_rs::VALIDATIONS.iter().any(|e| e.spec == CANARY_SPEC));
    }
    ...
    fn canary_detects_a_stripped_registry() {
        ...
        assert!(!triple_is_present(&lid_rs::SPECS, &lid_rs::IMPLEMENTATIONS, &[]));
    }
}
