...
mod tests {
    ...
    //!
    //! **[`Capture`] is the exception, and it is exercised through the real
    //! API.** It is the one item whose subject is `tracing`'s own protocol —
    ...
    use super::{
        Capture, MIN_INPUTS, SPAN_LEVEL, Span, TARGET, Trace, captured, cited_claims,
    ...
    fn a_recorded_span_carries_the_fields_it_was_opened_with() {
        ...
        assert_eq!(
            ...
            [
                ...
                ("lid.feature".to_string(), "audit".to_string()),
            ]
        );
    }
