...
**Settled by what was built: the first, with a correction to the second's
premise.** The emission carries no claims; `observed`, `span_around` and
`capture_around` are held by the checks' claims and by `lid-rs-macros`'s own
Phase 5 red set for the attributes. But "reddened only after the cascade" turned
out to be expressible after all — just not by a phase agent. The two claims over
`Returned` were derived and reddened from the main session after the cascade,
with check 12 standing in for the `todo!()`, and the sequence is written down as
the sixth hand-landing item. What no phase's red set can express is not the
same as what cannot be done.
...
| `lid_rs::validate::Recording` | The capture's state while it is installed: the spans so far and the stack of entered ones. Private, and the only part of the capture that is not a published promise — `captured` reads the spans back out as a `Trace` and nothing outside the module names the stack. |
...
| `Fields::record_debug`, `Fields::record_str` | The two arms `tracing` routes a field value between, both feeding `take`. Separate because a `Debug` rendering of a string quotes it, and `lid.claims` through that arm would split into a first claim carrying a quote and a last one carrying another. Neither is cited: the destination is `take`'s claim and the rendering is `tracing`'s dispatch, not a decision of this slice. Check 12 found that every field case passed a `&str`, so one of the two was never run. |
...
| `Returned::of` | The guard for one span, `const` and held for the length of the body it watches. |
...
| `lid_rs::validate::citing` | The spans of a trace that cite one claim: the selection all eight checks make before they read anything, spelled once. Uncited, because every question it answers belongs to its caller — check 23 is its empty case and each rule reads what it selected. |
...
| `lid_rs::validate::line` | One span's line: its name, then each claim it cites with `mark`'s verdict beside it, then its outcome, indented by `depth`. Three of the renderer's claims are here, because a line is where they are visible. |
| `lid_rs::validate::depth` | How many spans stand above one span — the length of its parent chain, and the indentation `line` draws. Bounded by the trace's length, so a `parent` pointing at itself prints a wrong depth rather than hanging the renderer: a wrong answer a test can see instead of a run that never ends. Uncited for `citing`'s reason. |
| `lid_rs::validate::mark` | The `✓ promised` mark for one claim on one line, or nothing. Beside the claim rather than at the end of the line, because a span cites as many claims as its `#[implements]` names and an end-of-line mark could not say which was kept. |
| `lid_rs::validate::finding` | The dispatch under `report`: `unreached`, `wrong outcome`, or the empty string where neither check has anything. Uncited for `citing`'s reason, and the same reading `claim::is_free` takes under `claim::free`. |
...
| `lid_rs_macros::expand::observed` | The cascade's one flow decision: `Verb::Implements` takes `span_around` and `Verb::Validates` takes `capture_around`. It is what makes the two attributes different at runtime rather than only in the registry. |
| `lid_rs_macros::expand::span_around` | What `#[implements]` wraps a body in: a span at `TARGET` and `SPAN_LEVEL` named for the fn, carrying `lid.claims` joined at `SEPARATOR` and an empty `lid.outcome`; then the entered guard and a `Returned` over the span, with the body left in its tail position. The join is written *inside* the `span!` invocation, where `tracing` skips a disabled callsite's field expressions — bound to a local above it, `cargo-lid-rs`'s suite went from 41s to not finishing. |
| `lid_rs_macros::expand::capture_around` | What `#[validates]` wraps a body in: `captured`, then `report` over the trace and the cited claims, then an `eprintln!` if the report is not empty, then the answer — or `resume_unwind` of the panic, so a failing validator still fails after its tree has printed. |
| `fixture` and `EVENT_DRIVEN`, `UNWANTED`, `OPTIONAL`, `STATE_DRIVEN`, `UBIQUITOUS`, `OTHER` | Test support: one `const fn` and the five `SpecMeta` values the rules are asked about, each naming a different claim so that a body reading every span rather than the citing ones answers differently. `OTHER` is the claim no rule here is asked about, which is what makes that visible. |
| `span`, `closed`, `carrying`, `under`, `trace` | Test support: the builder and its three modifiers, and the wrapper that makes a `Trace` of them. Taking the tree as plain data was for this — every check outside `Capture`'s own is handed a tree assembled here rather than recorded. |
| `names`, `parents` | Test support: the two projections a case asserts a recorded tree through, so an assertion names what it is about instead of indexing twice. |
| `recorded` | Test support: installs a `Capture` under `dispatcher::with_default`, runs a body that opens real spans, and hands back what the capture recorded. The one helper that exercises `tracing`'s protocol rather than this module's data. |
| `FixtureCallsite`, `LID_CALLSITE`, `LID_META`, `APP_CALLSITE`, `APP_META` | Test support: a `Callsite` and two `&'static Metadata<'static>` values, one at `TARGET` and one not. `register_callsite` takes a `'static` metadata that no `span!` macro hands a caller and no borrow of a local can be, so its two cases need callsites declared beside them — still `tracing`'s own types, and still not a mock. |
| `event_driven_traces`, `unwanted_traces`, `optional_traces`, `state_driven_traces`, `ubiquitous_spans`, `ubiquitous_traces` | Test support: one satisfied and one unsatisfied trace per rule, built once and read by both the rule's own case and the dispatch's. The dispatch's five negative cases and its one all-satisfied case share these, which is what stops a rule and the arm that calls it from being tested against different trees. |
