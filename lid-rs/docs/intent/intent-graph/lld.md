...
`lid-rs` ships the checks themselves, not a recipe for writing them: pure
...
`#[cfg(test)]` module; `lid-rs` invokes the same macro on itself.

## The registry is binary-global; the checks are crate-scoped

A consumer's test binary links `lid-rs`, so its registries contain `lid-rs`'s specs
alongside its own. `lid-rs`'s implementation edges ride along in the library, but
its `#[validates]` edges are `#[cfg(test)]` and exist only in `lid-rs`'s own test
...
| Emitted tests | Plain `#[test]`s, uncited | Emitted tests carrying `#[validates]` of the graph claims | The claims about check *behaviour* are validated by synthetic-input tests in `lid-rs`; emitted tests are each consumer's enforcement instance, and citing lid's specs from every consumer would scatter cross-crate edges that mean nothing to the consumer's own graph. |
...
- `lid-rs/docs/intent/registry/lld.md` — slice parameterization and the canary.
