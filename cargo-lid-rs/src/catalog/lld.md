...
| Measuring the cargo spawn | `cargo_output` is exempt from check 12; `spawned` is not | Exempt both; exempt neither and spawn a build in a unit test; drop `run` from the slice | The bar `.cargo/mutants.toml` sets is that an exempted function hold no branch of the slice's own. `cargo_output` builds a command, runs it and captures both streams — no branch, and a validation of it is a build. `spawned` holds a four-arm dispatch over what an invocation is, and two of those arms spawn nothing: the `sync` comparison is this binary's own work, and an unbuilt entry answers the sentence `unbuilt_sentence` composes. Those two are observable in process, so `spawned` is tested rather than exempted. Exempting it would have exempted a decision, which is what the bar exists to prevent. |
