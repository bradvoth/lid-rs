...
// ---- sync (docs/intent/sync/lld.md) -------------------------------------------

/// When `sync` runs, the skill shall be read from the `lid-rs` package the
/// project's manifest resolves — registry or path — never from the tool's
/// own build.
#[derive(Spec)]
pub struct TheSkillComesFromTheResolvedLidRsDependency;

/// When `sync` writes the skill, it shall write
/// `<workspace_root>/.claude/skills/lid-rs/SKILL.md`, creating directories,
/// and running it again shall change nothing.
#[derive(Spec)]
pub struct TheSkillCopyLivesAtTheWorkspaceRoot;

/// When `sync --check` runs and the project's copy is absent or differs from
/// the dependency's skill, it shall fail naming the file and write nothing;
/// when identical it shall succeed; any other flag shall be rejected by name.
#[derive(Spec)]
