...
| Measuring the two prints | Exempt from check 12 by `exclude_re`, as `read_line` is | A seam on each: `announce` answering with the line it printed, `narrate` answering with the text; inlining both at their call sites so no function exists to mutate | The gate asked the question Deferred 6 reserved, and no honest seam answers it. `narrate`'s signature is pinned by the canopy client's `Narrator`, which returns nothing, so a seam there means widening another slice's type for this one's benefit — the thing that slice's LLD refuses. `announce` could answer with its line, but the assertion would be that `announced` answers what `announced` answers, which is the test that pretends to observe a print. Inlining removes the functions and not the problem: `announce` has two call sites, so inlining duplicates the idiom, and a `narrate` written as a closure hides which narrator a session is driven with, which is the thing the tests do observe. Both hold no branch of the slice's own — every decision is in `announced`, `announcing` and `argument_named`, and `narrate` is the identity — so both are the pure I/O sequencing the exemption is for. |
...
6. Whether `TheModelsSettledAnswerIsPrinted` should gain a seam a test can
   observe — the last of the printing claims still open, the other two
   having been settled at a gate (§ Decisions & Alternatives, "Measuring the
   two prints": check 12 reached `narrate` and `announce`, both survived,
   and no seam was reachable that did not either widen another slice's type
   or assert a tautology). It prints the model's text verbatim, so the line
   function that would carry it is the identity;
