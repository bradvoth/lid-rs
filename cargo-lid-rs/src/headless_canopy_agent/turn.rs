...
use super::door::{Door, Offered, Settings, Started};
...
pub enum Halt {
    ...
    Quiet,
    /// The door refused a request: its sentence.
    Refused(String),
}

/// Lands `message` as one `app.client.user_message` and follows the tail
/// by parked reads — `wait` 25 s, clamped to the credential's remaining
/// life, the credential refreshed within five seconds of its expiry —
/// classifying each record by kind, one `match`, and decoding its body into
/// [`Payload`], [`Forward`], [`Responded`], [`Denied`], or [`Halted`] there
/// and nowhere else: a payload is held; a forward is paired and its tool
/// run and answered; a denial is counted as a refusal; a response without
/// `tool_uses` settles the turn, one with `terminal` lands the message once
/// more and a second stops the run; a halt ends the session with its
/// reason; fifteen quiet minutes end it too; and the door's refusal of any
/// send, tail, or refresh along the way ends it with the door's sentence.
#[implements(
    spec::TheTailIsFollowedByParkedReadsWithinTheCredentialsLife,
    spec::TheCredentialIsRefreshedBeforeItExpires,
...
/// (the completion's addressee), and the call's arguments.
#[derive(Debug, Clone, PartialEq)]
pub struct Pairing {
...
/// — the forward's digest, `to` the payload's producer, `outcome` `success`
/// with the result or `error` with the message — and its idempotency key,
/// `:<forward cursor>:65534:0`.
#[implements(spec::ACompletionAnswersThePayloadsProducer, spec::ACompletionsIdempotencyKeyIsTheForwardsCursor)]
