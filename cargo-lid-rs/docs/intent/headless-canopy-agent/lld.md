...
| `grep` | `pattern`, optional `path` (default the root), `glob` | A literal, case-sensitive substring search over files under `path`, never entering `target/` or `.git/` | `path:line: text` per match, capped at 200 lines |
| `glob` | `pattern` | Files matching a glob under the root, never under `target/` or `.git/`; the pattern is the path the confinement rule below applies to (absolute, or any `..` component, refused before the verdict), and a match that resolves outside the root through a symlink is omitted from the answer | The paths, sorted |
...
| `SKIPPED`, `skipped(component) -> bool` | The two directory names `grep` and `glob` never enter, `target` and `.git`, as one predicate both walks ask |
