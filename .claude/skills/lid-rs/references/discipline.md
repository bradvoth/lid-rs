...
| 4, 6 | A loop's end depends on arithmetic — a counter incremented in the body, a bound tested inside it | Bound it by the loop's shape: `for _ in 0..N`, with the body free to `break` early. A loop that ends only if `attempt += 1` runs *and* a predicate eventually says stop has two single points of failure, and breaking either is a hang, not a wrong answer — no test fails and the run times out. Clippy has no lint for this (checked across all, pedantic, nursery, restriction: only `while_let_loop` fires, and its advice keeps the hazard). Check 12 finds it, as a TIMEOUT, an hour later. |
