...
impl Verb {
    ...
    fn slice(&self) -> TokenStream {
        match self {
            Verb::Implements => quote!(::lid::IMPLEMENTATIONS),
            Verb::Validates => quote!(::lid::VALIDATIONS),
        }
    }
}
...
fn ensure_unit_struct(item: &DeriveInput) -> syn::Result<()> {
    ...
    if !unit || !item.generics.params.is_empty() {
        return Err(syn::Error::new_spanned(
            &item.ident,
            "lid: derive(Spec) applies to non-generic unit structs only — a claim has no runtime shape",
        ));
    }
    Ok(())
}
...
pub fn citation(args: TokenStream, item: TokenStream, verb: Verb) -> syn::Result<TokenStream> {
    ...
    Err(syn::Error::new_spanned(
        item,
        "lid: citations apply to fns, structs, and enums",
    ))
}
...
pub fn foreign_key(args: TokenStream, item: TokenStream) -> syn::Result<TokenStream> {
    let id: LitStr = syn::parse2(args)
        .map_err(|e| syn::Error::new(e.span(), "lid: expected a single foreign-ID string literal"))?;
...
fn parse_spec_paths(args: TokenStream) -> syn::Result<Vec<Path>> {
    ...
    let paths: Punctuated<Path, Token![,]> =
        Punctuated::parse_terminated.parse2(args).map_err(|e| {
            syn::Error::new(e.span(), "lid: expected a comma-separated list of spec paths")
        })?;
    if paths.is_empty() {
        return Err(syn::Error::new(
            span,
            "lid: cite at least one spec, or remove the attribute",
        ));
    }
...
fn ensure_plain_path(path: &Path) -> syn::Result<()> {
    if path.segments.iter().any(|s| !s.arguments.is_none()) {
        return Err(syn::Error::new_spanned(
            path,
            "lid: spec paths are plain paths — a claim type takes no generic arguments",
        ));
    }
    Ok(())
}
