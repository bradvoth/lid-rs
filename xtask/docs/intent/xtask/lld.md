...
they are absent from the xtask binary itself, and absent from `lid-rs` as linked
...
| JSON / metadata parsing | `serde_json` untyped `Value`; config via `cargo metadata` | `toml` crate for Cargo.toml; typed serde derives | One parsing dependency instead of two: `cargo metadata` re-serves `[workspace.metadata.lid_rs]` as JSON. Untyped access keeps the surface small. |
...
| Test scope per mutant | `--test-workspace true` | Engine default (test only the mutated package) | Found by a surviving mutant: a broken `derive` in the proc-macro crate is killed only by `lid-rs`'s tests, and the package-scoped default never runs them. Cross-crate kill paths are the norm here. |
