...
every gate. The differential pass ([§4.4](https://bradvoth.github.io/lid-rs/spec/gates.html)) exists for that case, and
the system's success condition is that it is the only thing left for it to do.
...
| 2 | **Broken doc link** | `cargo doc` with `-D rustdoc::broken_intra_doc_links` | A hand-written link in an LLD or doc comment dangles. Redundant with check 1 on purpose; this one also covers hand-written links. |
...
**The complexity threshold bounds what an untraced function can be.** At complexity 1 it
...
.claude/agents/lid-rs-*.md     the phase agents (one per committing phase) and
                               the reviewer — edit-only and read-only tool sets,
...
agent (§8) cannot run a command, edit outside its phase's paths, or commit;
...
`docs/intent/<slice>/compile-time-accepted` with its LLD; nothing detects
...
cargo lid-rs sync           # rewrites the skill, agents, and workflow from it
```

The gate's `cargo lid-rs sync --check` fails until every copy matches — after
a bump you have not synced, and after any local edit. The files are
tool-owned;
