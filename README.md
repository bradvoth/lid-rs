...
*Revision 3. Supersedes r2: purpose and scope promoted out of "limits",
configuration consolidated, the registry mechanism documented, and the
traced/untraced boundary given a real definition.*
...
The attribute expands to three things:
...
// 2. a const type-assertion — a bad path is a compile error, and a
//    #[deprecated] spec warns right here
const _: fn() = || {
    fn assert_spec<T: ::lid::Spec>() {}
    assert_spec::<crate::spec::ValidCredentialsYieldScopedSession>();
};

// 3. a link-time registry entry (see §5)
#[doc(hidden)]
#[allow(non_upper_case_globals, missing_docs, clippy::missing_docs_in_private_items)]
#[::lid::__private::linkme::distributed_slice(::lid::IMPLEMENTATIONS)]
static __LID_IMPL_a3f2: ::lid::Edge = ::lid::Edge {
    spec: ::std::any::type_name::<crate::spec::ValidCredentialsYieldScopedSession>(),
...
Effect (2) is what removes the parser. Name resolution is the compiler's, so
`use` renames, re-exports, and `#[cfg]` are handled correctly and for free.
Effect (3) makes the graph enumerable at runtime without anyone reading source.
...
Generated statics carry `#[doc(hidden)]` and `#[allow(non_upper_case_globals,
missing_docs, clippy::missing_docs_in_private_items)]` — without those, check 3
fires on every citation in the crate, catching code the user didn't write.
